...
public:
  ...
  static Eigen::VectorXd run(GradientFunc gradient_func, const Eigen::VectorXd& initial_guess,
                             const Eigen::VectorXd& initial_step_size, const Eigen::MatrixXd& limits,
                             std::shared_ptr<Config> conf);

  /// Conversion from the tuning space to user space
  static Eigen::VectorXd cvtFromTuningSpace(const Eigen::VectorXd& v, std::shared_ptr<Config> conf);
  /// Conversion from user space to tuning Space
  static Eigen::VectorXd cvtToTuningSpace(const Eigen::VectorXd& v, std::shared_ptr<Config> conf);
};
