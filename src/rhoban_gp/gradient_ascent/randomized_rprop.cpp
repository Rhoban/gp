...
namespace rhoban_gp
{
RandomizedRProp::Config::Config() : nb_trials(10), rprop_conf(new RProp::Config())
{
}
...
int RandomizedRProp::Config::write(std::ostream& out) const
{
  ...
  if (rprop_conf)
    bytes_written += rprop_conf->write(out);
...
Eigen::VectorXd RandomizedRProp::run(RProp::GradientFunc gradient_func, ScoringFunc scoring_func,
                                     const Eigen::MatrixXd& limits, const Config& conf)
{
  // DEBUG
  ...
  for (int trial = 0; trial < conf.nb_trials; trial++)
  {
    ...
    current_guess = RProp::run(gradient_func, initial_guess, initial_step_size, limits, conf.rprop_conf);
