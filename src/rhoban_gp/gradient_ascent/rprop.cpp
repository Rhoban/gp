...
{
RProp::Config::Config()
  : epsilon(std::pow(10, -6)), max_iterations(1000), eta_pos(1.2), eta_neg(0.5), tuning_space(TuningSpace::Normal)
{
}
...
void RProp::Config::fromJson(const Json::Value& v, const std::string& dir_name)
{
  ...
  if (tuning_space_str.size() > 0)
  {
    tuning_space = loadTuningSpace(tuning_space_str);
  }
}
...
Eigen::VectorXd RProp::run(std::function<Eigen::VectorXd(const Eigen::VectorXd)> gradient_func,
                           ...
                           const Eigen::MatrixXd& limits, std::shared_ptr<Config> conf)
{
  ...
  if (!conf)
    conf = std::shared_ptr<Config>(new Config());
  ...
  while (true)
  {
    ...
    if ((last_guess - guess).cwiseAbs().maxCoeff() < conf->epsilon)
      break;
...
Eigen::VectorXd RProp::cvtFromTuningSpace(const Eigen::VectorXd& v, std::shared_ptr<Config> conf)
{
  Eigen::VectorXd result = v;
  if (conf->tuning_space == TuningSpace::Normal)
    return result;
...
Eigen::VectorXd RProp::cvtToTuningSpace(const Eigen::VectorXd& v, std::shared_ptr<Config> conf)
{
  Eigen::VectorXd result = v;
  if (conf->tuning_space == TuningSpace::Normal)
    return result;
...
std::string to_string(RProp::TuningSpace tuning_space)
{
  switch (tuning_space)
  {
    case RProp::TuningSpace::Normal:
      return "Normal";
    case RProp::TuningSpace::Log:
      return "Log";
  }
